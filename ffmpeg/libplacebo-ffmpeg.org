...
-i input.mp4 \
-filter_hw_device vk \
-vf "hwupload,libplacebo=w=3840:h=2160:upscaler=lanczos:custom_shader_path=$HOME/Documents/ffmpeg-shaders/SSimSuperRes.glsl:disable_builtin=1:format=yuv420p,hwdownload,format=yuv420p" \
-c:v hevc_nvenc -preset p7 -tune hq -rc vbr -cq 20 -b:v 0 \
-c:a copy \
...
output_default="${input_name}-upscale.mp4"
