...
8) write a several hundred word description of the data as a photographic scene that can be used by artifical intelligence machine that can create images from a text prompt, which should start with the words "Create a photo realistic image of "
