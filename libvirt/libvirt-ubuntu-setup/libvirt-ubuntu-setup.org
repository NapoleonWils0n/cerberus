...
Note to install the windows subsystem for linux you need to uncheck Copy host CPU configuration
and under the model drop down edit the text so it says host-passthrough
...
*** set the video to virtio

change the video model from qxl to virtio
boot into windows and go to the settings app / display and change the display to 1920x1080
