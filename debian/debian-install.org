...
A Text Editor (like gedit): sudo apt install gedit
Network Manager (if not already installed): sudo apt install network-manager gnome-network-manager
(and then enable the service if needed: sudo systemctl enable NetworkManager and sudo systemctl start NetworkManager)
