...
#+begin_src sh
doas apt install abook aria2 bc curl dconf-editor ffmpeg fd-find git gnome-tweaks handbrake imagemagick kodi \
kodi-inputstream-adaptive libsox-fmt-all mpd mpc mpv mpv-mpris ncmpc oathtool openvpn pandoc pinentry-curses \
playerctl ripgrep sox task-spooler tmux transmission-gtk \
rsync wl-clipboard 
#+end_src
