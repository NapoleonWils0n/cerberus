...
#+begin_src sh
wget -c 'https://cdimage.debian.org/debian-cd/current/amd64/iso-cd/debian-13.2.0-amd64-netinst.iso'
#+end_src
...
#+begin_src sh
sudo cp debian-13.2.0-amd64-netinst.iso /dev/sda
#+end_src
...
add non-free-firmware non-free
...
deb-src http://deb.debian.org/debian/ trixie-backports main non-free-firmware non-free
...
#+begin_src sh
apt install linux-headers-$(uname -r) broadcom-sta-dkms
#+end_src

black list the old driver

This unloads the conflicting open-source modules

an loads the proprietary one you just installed

#+begin_src sh
modprobe -r b43 brcmsmac bcma ssb
modprobe wl
#+end_src
