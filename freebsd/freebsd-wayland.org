...
#+begin_src sh
doas dd if=FreeBSD-15.0-RELEASE-amd64-memstick.img of=/dev/da0 bs=1M conv=sync
#+end_src
...
#+begin_src sh
doas sysrc kld_list+="i915kms nvidia-modeset nvidia-drm nvidia_gsp_tu10x_fw linux linux64"
#+end_src
...
install the package updates in the existing boot environment.
...
#+BEGIN_SRC sh
zfs snapshot -r zroot@15.0-p0
#+END_SRC

If you want to revert to the boot environment you created

activate the boot environment you want to revert to
...
Reboot. After the reboot, you’ll see the environment is running.
...
#+BEGIN_SRC sh
doas sysrc local_unbound_enable="NO"
#+END_SRC

enable the unbound package dns server

#+BEGIN_SRC sh
doas sysrc unbound_enable="YES"
#+END_SRC
...
#+BEGIN_SRC sh
doas pkg install ca_root_nss
#+END_SRC

*** create the root.key

create the root.key if it doesnt exist

change persmissions on the unound directory

#+begin_src sh
doas chown unbound:wheel /usr/local/etc/unbound
#+end_src

create the root.key

#+begin_src sh
doas -u unbound unbound-anchor -a "/usr/local/etc/unbound/root.key"
#+end_src
...
#+BEGIN_SRC sh
doas vi /usr/local/etc/dnscrypt-proxy/dnscrypt-proxy.toml
#+END_SRC
...
#+BEGIN_SRC sh
doas sysrc dnscrypt_proxy_enable="YES"
doas sysrc dnscrypt_proxy_pidfile="/var/run/dnscrypt-proxy.pid"
doas sysrc dnscrypt_proxy_logfile="/var/log/dnscrypt-proxy.log"
#+END_SRC
...
#+begin_src sh
doas service dnscrypt-proxy restart
#+end_src
...
#+BEGIN_SRC sh
doas poudriere jail -c -j freebsd_15-0x64 -v 15.0-RELEASE
#+END_SRC
...
#+BEGIN_SRC sh
doas vi /usr/local/etc/poudriere.d/freebsd_15-0x64-make.conf
#+END_SRC
...
#+BEGIN_SRC sh
doas poudriere options -j freebsd_15-0x64 -p local -f /usr/local/etc/poudriere.d/port-list
#+END_SRC
...
#+BEGIN_SRC sh
doas poudriere options -c -j freebsd_15-0x64 -p local -f /usr/local/etc/poudriere.d/port-list
#+END_SRC

bulk build with no user input

#+BEGIN_SRC sh
doas poudriere bulk -j freebsd_15-0x64 -p local -f /usr/local/etc/poudriere.d/port-list
#+END_SRC
...
#+BEGIN_SRC sh
doas poudriere jail -u -j freebsd_15-0x64
#+END_SRC
...
#+BEGIN_SRC sh
Poudriere: {
    url: "file:///usr/local/poudriere/data/packages/freebsd_15-0x64-local"
    mirror_type: "srv",
...
*** linux-widevine-cdm install

#+begin_src sh
doas pkg install linux-widevine-cdm
#+end_src
