...
#+begin_src sh
poudriere ports -c -p quarterly -m git+https -B 2026Q2
#+end_src
...
#+BEGIN_SRC sh
poudriere jail -c -j freebsd_15-0x64 -m git+https -v main
#+END_SRC
...
We can use the -p flag to name our ports tree. We will call our tree "quarterly" as it accurately summarizes the use of this tree (the "head" or most up-to-date point of the tree). We will be updating it regularly to match the most current version of the ports tree available:

#+BEGIN_SRC sh
poudriere ports -c -p quarterly
#+END_SRC
...
#+BEGIN_SRC sh
vi /usr/local/etc/poudriere.d/freebsd_15-0x64-make.conf
#+END_SRC
...
#+BEGIN_SRC sh
poudriere options -j freebsd_15-0x64 -p quarterly -f /usr/local/etc/poudriere.d/port-list
#+END_SRC
...
#+BEGIN_SRC sh
poudriere options -c -j freebsd_15-0x64 -p quarterly -f /usr/local/etc/poudriere.d/port-list
#+END_SRC

bulk build with no user input

#+BEGIN_SRC sh
poudriere bulk -j freebsd_15-0x64 -p quarterly -f /usr/local/etc/poudriere.d/port-list
#+END_SRC
...
#+BEGIN_SRC sh
poudriere jail -u -j freebsd_15-0x64
#+END_SRC
