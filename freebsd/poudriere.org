...
#+BEGIN_SRC sh
# poudriere options -j freebsd_15-0x64 -p HEAD -f /usr/local/etc/poudriere.d/port-list
#+END_SRC
...
#+BEGIN_SRC sh
# poudriere options -c -j freebsd_15-0x64 -p HEAD -f /usr/local/etc/poudriere.d/port-list
#+END_SRC

bulk build with no user input

#+BEGIN_SRC sh
# poudriere bulk -j freebsd_15-0x64 -p HEAD -f /usr/local/etc/poudriere.d/port-list
#+END_SRC
...
#+BEGIN_SRC sh
Poudriere: {
    url: "file:///usr/local/poudriere/data/packages/freebsd_15-0x64-HEAD"
    mirror_type: "srv",
