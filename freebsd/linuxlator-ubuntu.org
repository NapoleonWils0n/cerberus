...
check for missing libraies using ldd

on the freebsd host run ldd and the path to the resolve binary in the chroot

#+begin_src sh
ldd /compat/ubuntu/opt/resolve/bin/resolve > ldd-resolve
#+end_src

grep for 'not found'

#+begin_src sh
ldd /compat/ubuntu/opt/resolve/bin/resolve | grep 'not found' > ldd-notfound
#+end_src

search for the missing libraries

#+begin_src sh
sudo find /compat/ubuntu -name libpthread.so.0
#+end_src

*** resolve.conf

create the libmap.conf file
...
#+begin_src conf
[/compat/ubuntu/opt/resolve/bin/resolve]
libOpenCL.so.1        /compat/ubuntu/usr/lib/x86_64-linux-gnu/libOpenCL.so.1
libbz2.so.1           /compat/ubuntu/lib/x86_64-linux-gnu/libbz2.so.1
libc.so.6             /compat/ubuntu/lib/x86_64-linux-gnu/libc.so.6
libcrypt.so.1         /compat/ubuntu/lib/x86_64-linux-gnu/libcrypt.so.1
libdl.so.2            /compat/ubuntu/lib/x86_64-linux-gnu/libdl.so.2
libm.so.6             /compat/ubuntu/lib/x86_64-linux-gnu/libm.so.6
libpthread.so.0       /compat/ubuntu/lib/x86_64-linux-gnu/libpthread.so.0
libresolv.so.2        /compat/ubuntu/lib/x86_64-linux-gnu/libresolv.so.2
libudev.so.1          /compat/ubuntu/usr/lib/x86_64-linux-gnu/libudev.so.1
libz.so.1             /compat/ubuntu/lib/x86_64-linux-gnu/libz.so.1
ld-linux-x86-64.so.2  /compat/ubuntu/lib64/ld-linux-x86-64.so.2
#+end_src

copy the resolve.conf to /usr/local/etc/libmap.d

#+begin_src sh
sudo cp resolve.conf /usr/local/etc/libmap.d/
#+end_src
