...
we need to move the compat and linux directories because when we create the home datasets
...
zfs create zroot/compat/ubuntu
#+END_SRC

set the zfs mountpoint

#+begin_src sh
zfs set mountpoint=/compat zroot/compat
zfs set mountpoint=/compat/linux zroot/compat/linux
zfs set mountpoint=/compat/ubuntu zroot/compat/ubuntu
#+end_src
...
copy files from bak-compat to the dataset before setting the mount point and mounting the dataset

#+BEGIN_SRC sh
cp -Rpv /compat/linux/ /compat/linux
#+END_SRC
...
we use /compat/linux/ to copy the contents of the documents directory  
