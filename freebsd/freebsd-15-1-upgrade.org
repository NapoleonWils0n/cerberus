...
note when do i need to remove temporary kld list and re enable the original
...
*** 2) First, update pkg(8) itself to ensure you have the latest version:

#+begin_src sh
doas pkg upgrade -yr FreeBSD-ports pkg
#+end_src

*** 3) Second, upgrade the base system
...
*** 4) Then upgrade the base system:
...
*** 9) check for configuration file updates that may have been installed as ‘.pkgnew‘ files:
...
#+begin_src sh
doas rm -r /tmp/upgrade-15.1
#+end_src

*** 10) activate the new boot env
...
*** 12) How to Determine Your Base System Type
