...
#+BEGIN_SRC sh
doas poudriere options -j freebsd_15-1x64 -p local -f /usr/local/etc/poudriere.d/port-list
#+END_SRC
...
#+BEGIN_SRC sh
doas poudriere options -c -j freebsd_15-1x64 -p local -f /usr/local/etc/poudriere.d/port-list
#+END_SRC
...
press 3 at the boot menu

#+begin_src sh
disable-module nvidia-drm
boot
#+end_src
...
# 1. Update your databases using the custom repository directory flags
#+begin_src sh
doas pkg -o REPOS_DIR=/etc/pkg,/usr/local/etc/pkg/repos -o IGNORE_OSVERSION=yes update -f
#+end_src

# 3. Drop the legacy, crashing driver module
#+begin_src sh
doas pkg delete -y 'nvidia*kmod*'
#+end_src

# 2. Tell pkg exactly where the configs are when calling the kmods repo upgrade
#+begin_src sh
doas pkg -o REPOS_DIR=/etc/pkg,/usr/local/etc/pkg/repos upgrade -y -r FreeBSD-ports-kmods
#+end_src

# 4. Pull down the fresh 15.1 module matching your new kernel framework
#+begin_src sh
doas pkg -o REPOS_DIR=/etc/pkg,/usr/local/etc/pkg/repos install -y nvidia-drm-kmod nvidia-driver nvidia-settings
#+end_src

# 5. Perform the final structural package sync across the board
#+begin_src sh
doas pkg -o REPOS_DIR=/etc/pkg,/usr/local/etc/pkg/repos upgrade -y
#+end_src

delete the tmp kld list
uncomment the original and reboot
