...
htdemucs_ft fined tuned model take longer but better results

wav

#+begin_src sh
demucs --float32 --two-stems=vocals --shifts=10 -n htdemucs_ft input.wav
#+end_src

mp3 

#+begin_src sh
demucs --mp3 --two-stems=vocals --shifts=10 -n htdemucs_ft input.wav
#+end_src
...
#+begin_src sh
demucs --float32 --two-stems=vocals --shifts=10 input.wav
#+end_src

mp3 

#+begin_src sh
demucs --mp3 --two-stems=vocals --shifts=10 --overlap=0.1 input.wav
#+end_src

*** mdx_extra_q

old model fast but leave some trace of the music in the vocals

#+begin_src sh
demucs --mp3 --two-stems=vocals -n mdx_extra_q --shifts=10 --overlap=0.1 input.wav
#+end_src
