...
podman volume create searxng_config
...
#+begin_src sh
podman run -d --name searxng --network searxng --volume searxng_config:/etc/searxng:rw -p 8080:8080 --env SEARXNG_BASE_URL=http://searxng:8080 --env SEARXNG_REDIS_URL=redis://redis:6379/0 docker.io/searxng/searxng:latest
#+end_src
