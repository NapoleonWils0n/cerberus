...
add the require statements after the combobulate use package block
...
place your cursor after the ) in the require statements after and evalulate them by running
...
C-c o B q command should be run in the buffer containing the shell script that you're editing.
